#!/usr/bin/env python3
"""
Build county × week rollup from weather+PUE/WUE data.

Reads the (large) hourly weather + PUE/WUE CSV, aggregates to one row per
county per week (ISO week 1–53) with mean temp, RH, pressure, AE_PUE, AE_WUE, WEC_PUE, WEC_WUE.
Writes to the parent project's data folder (Counties+Zones Database/data/).

Usage (run from Counties+Zones Database):
  python "Dashboard Tool/scripts/build_county_week_rollup.py"
  python "Dashboard Tool/scripts/build_county_week_rollup.py" --input /path/to/big.csv --output data/county_week_rollup.csv
"""
...
DEFAULT_INPUTS = [
    PARENT_FOLDER / "FINAL_counties_with_pue_wue.csv",
    PROJECT_ROOT / "PUE WUE SIM" / "weather_hourly_with_counties_with_pue_wue.csv",
]
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "county_week_rollup.csv"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Build county × week rollup for dashboard")
    ...
    print(f"Reading {inp} in chunks (aggregating by week)...")
    chunks = []
    for i, chunk in enumerate(pd.read_csv(inp, chunksize=args.chunk, low_memory=False)):
        if "timestamp" in chunk.columns:
            ...
            try:
                ...
                chunk["week"] = iso.week.fillna(1).astype(int)
                chunk["year"] = iso.year.fillna(2024).astype(int)
            except AttributeError:
                chunk["year"] = ts.apply(lambda x: x.year if pd.notna(x) and hasattr(x, "year") else 2024)
                chunk["week"] = ts.apply(lambda x: x.isocalendar()[1] if pd.notna(x) and hasattr(x, "isocalendar") else 1)
        else:
            chunk["week"] = 1
            chunk["year"] = 2024
        ...
        chunk["week"] = chunk["week"].fillna(1).astype(int)
        agg_cols = [c for c in ["temp_c", "rh_pct", "pressure_hpa", "AE_PUE", "AE_WUE", "WEC_PUE", "WEC_WUE"] if c in chunk.columns]
        group_cols = [c for c in ["county_fips", "county_name", "state_abbr", "state_full", "year", "week"] if c in chunk.columns]
    ...
    group_cols = [c for c in ["county_fips", "county_name", "state_abbr", "state_full", "year", "week"] if c in df.columns]
    ...
    print(f"  Counties: {df['county_fips'].nunique()}, Weeks: {df['week'].nunique()}")
